...
public class Map implements ActionListener , KeyListener {
	// Window for game with graphics and buttons.
	public JFrame frame= new JFrame("J*Man!!!"); // The GUI window
	public JFrame instructionFrame = new JFrame("Instruction");
	...
	public Map(int h, int w, int bl, int wa, int pi){
		...
		bOk.setPreferredSize(buttondim);

		// Register this as an action listener for all buttons.
		bOk.addActionListener((e) -> { instructionFrame.setVisible(false);});
		...
		menuHelp.add(instructions);
		newGame.addActionListener( (e) -> { restartGame();});
		instructions.addActionListener( (e) -> { instructionFrame.setVisible(true);});
		...
		instructionFrame.getContentPane().add(instructBox, BorderLayout.CENTER);
		instructionFrame.getContentPane().add(bOk, BorderLayout.SOUTH);
		instructionFrame.pack();
		instructionFrame.setLocation( 200, 30);
	...
	private void addInstructions(Box instructBox) {
		instructBox.add(new JLabel(" Use the keyboard up down right left to direct J*Man ", SwingConstants.LEFT));
		instructBox.add(new JLabel("(the star like piece) to capture the other colored pieces.", SwingConstants.LEFT));
